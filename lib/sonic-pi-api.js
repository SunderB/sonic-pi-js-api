...
class SonicPiAPI {
  ...
  */
  preprocess_code(code) {
    ...
    {
      processed_code = "use_debug false #__nosave__ set by user preferences.\n" + processed_code;
    }

    if (!this.settings.log_cues)
    {
      processed_code = "use_cue_logging false #__nosave__ set by user preferences.\n" + processed_code;
    }

    if (this.settings.check_args)
    {
      processed_code = "use_arg_checks true #__nosave__ set by user preferences.\n" + processed_code;
    }

    if (this.settings.enable_external_synths)
    {
      processed_code = "use_external_synths true #__nosave__ set by user preferences.\n" + processed_code;
    }

    if (this.settings.enforce_timing_guarantees)
    {
      processed_code = "use_timing_guarantees true #__nosave__ set by user preferences.\n" + processed_code;
    }
    ...
    processed_code = `use_midi_defaults channel: "${midi_channel} #__nosave__ set by user preferences."\n` + processed_codeode;
   ...
   */
  run_code(code) {
    processed_code = this.preprocess_code(code);
